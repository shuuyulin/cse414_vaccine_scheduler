...
from db.SQLs import show_appointment_caregiver_details
...
class Caregiver:
    ...
    def get(self):
        return_ob = None
        curr_salt, curr_hash = services.get_caregiver_by_name(self.username)
    ...
    def save_to_db(self):
        services.insert_caregiver(self.username, self.salt, self.hash)

    # Insert availability with parameter date d
    def upload_availability(self, d):
        services.insert_avaliablities(d, self.username)

    def show_appointments(self) -> list[list]:
        sql = show_appointment_caregiver_details
